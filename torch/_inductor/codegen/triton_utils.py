...
def signature_to_meta(
    ...
    *,
    ...
    # size_dtype is based on iteration bounds, but size args like ks may
    # be multiplied in the kernel body (e.g., ks0*ks1 in torch.mean) and
    # overflow int32. Force int64 for symbols from tensors with large numel.
    #
    # Skip for block pointers (no 64-bit indexing support:
    # https://gist.github.com/shunting314/6a41c776171720ce4561f202dcde0ad6)
    # and templates (e.g., flex attention which uses block pointers).
    int64_symbols: set[sympy.Symbol] = set()
    if not config.triton.use_block_ptr and not is_template:
        int32_max = torch.iinfo(torch.int32).max
        for val in V.graph.graph_inputs.values():
            if not hasattr(val, "get_size"):
                continue  # skip scalars
            size = val.get_size()
            if not V.graph.sizevars.expect_true(sympy.prod(size) <= int32_max):
                for s in size:
                    int64_symbols.update(s.free_symbols)

    def _decide_tl_dtype(arg):
        if (
            isinstance(arg, SizeArg)
            and arg.expr is not None
            and arg.expr.free_symbols.intersection(int64_symbols)
        ):
            return "tl.int64"
        return size_dtype
