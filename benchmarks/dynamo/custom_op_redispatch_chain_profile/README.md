# Where does Richard's redispatch-chain prototype lose ~5μs?

Profile harness for the no-grad path of `torch.library.custom_op`'s fast path. Compares the current PR (`reduce-custom_op-overhead`, inlined kernel call) against Richard's prototype (`zou3519`'s "redispatch chain in Python", commit `2bf17e99a1d`).
...
Instead, I built **nine variants of the no-grad path bottom-up**. Each variant is an independent, self-contained `fast_call` body that contains *exactly* the ingredients listed for it, and nothing more. They share the same prelude (the `_C._custom_op_fast_path_check` C++ guard, kwargs/disabled/fn lookup), so the prelude cost cancels out of every delta.
...
v8_autograd_impl_full   : + recompute is_grad_enabled() and _any_requires_grad(*args)  -- == Richard's prototype
```
...
Top-down (short-circuit Richard's full path at progressively earlier points) gives you the cumulative cost at each profile point. Bottom-up (this approach) gives you the *marginal* cost of each ingredient. The marginal cost is what you actually want when deciding which ingredients to keep or remove.
...
### Faithfulness to Richard's prototype
...
Validation: `v1_alias` ≈ current PR no-grad (3,873 vs 3,451 measured Apr 23, the ~400 ns extra is closure indirection from `body(...)`); `v8_autograd_impl_full` ≈ Richard's prototype (8,422 vs 8,735 measured Apr 23, ~300 ns less for the same reason). They roughly cancel and the per-step deltas track the underlying cost.
...
- `SLACK_REPLY.md` — drafted reply to Richard.
